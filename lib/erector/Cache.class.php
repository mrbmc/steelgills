...
class Cache 
{
	...
	public $type = "memcache";//options are 'memcache','file','nosql'
	public $directory;
	public $lifetime = 3;//in seconds
	private $request;
	...
	private function __construct($_type) 
	{
		...
		if($this->type=="memcache" && class_exists("Memcache")) {
			...
			if(!$this->memcache->connect($this->servername, $this->serverport)) {
				Debugger::trace("could not connect to memcache daemon");
				$this->memcache = false;
			}
		} elseif ($this->type=="file") {
			if(!is_writable($this->directory))
				Debugger::trace("could not write to cache directory");
		}
	}
	...
	public function set ($request,$response,$lifetime=300) {
		$action = "set_" . $this->type;
		if($lifetime>0)
			$this->lifetime = $lifetime;
		return $this->$action($request,$response);
	}

	private function get_file ($request)
	{
	...
	private function set_file ($key, $response)
	{
		$file = $this->directory . "/" . $key . ".cache";
	...
	private function get_memcache ($request) {
		...
		if(!$cached) {
			return false;
//			Debugger::trace('uncached:'.$request);
//			$cache = $this->load($request);
//			if($this->memcache)
//				$this->set($key,$cache);
		} else {
		...
		if($return){
			if(is_array($headers)){
				...
				if($error) // failed to connect with host
					$buffer="Fail: ".Debugger::trace($s->error);
				elseif(preg_match("/404/",$h['status'])) // page not found
					$buffer="Fail: ".Debugger::trace("Page Not Found");
				else
					$buffer = $return;
			} else {
				$buffer="Fail: ".Debugger::trace("An unknown error occurred.");
			}
		}else{
			$buffer="Fail: ".Debugger::trace("An unknown error occurred.");
		}
